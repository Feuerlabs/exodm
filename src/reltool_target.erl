...
merge_apps(#rel{name = RelName,
    ...
    MergedApps2 = do_merge_apps(RelName, RelApps, Apps, RelAppType,
				lists:reverse(MergedApps)),
    io:fwrite("MergedApps2(~p) = ~p~n", [RelName, [A#app.name || A <- MergedApps2]]),
    Embedded =
	[A#app.name || A <- Apps,
    ...
    io:fwrite("Embedded(~p) = ~p~n", [RelName, Embedded]),
    MergedApps3 = do_merge_apps(RelName, Embedded, Apps, EmbAppType,
				lists:reverse(MergedApps2)),
    io:fwrite("MergedApps3(~p) = ~p~n", [RelName, [A#app.name || A <- MergedApps3]]),
    MergedApps3.
    %% Sorted = sort_apps(MergedApps3),
    %% io:fwrite("Sorted(~p) = ~p~n", [RelName, [A#app.name || A <- Sorted]]),
    %% Sorted.
...
do_merge_apps(RelName, [#rel_app{name = Name} = RA | RelApps], Apps, RelAppType, Acc) ->
	...
	false ->
	    ...
	    Acc2 = [MergedApp | Acc],
	    do_merge_apps(RelName, RelApps ++ MoreNames, Apps, RelAppType, Acc2)
    end;
